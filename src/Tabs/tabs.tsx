...
export const CSSTabs = ({
  ...
  return (
    <nav
      ref={navRef}
      className="flex flex-shrink-0 items-center relative z-0 py-2 border-b"
      ...
      {tabs.map((item, i) => {
        return (
          <button
            key={i}
            className={cn(
              "text-md relative rounded-md flex items-center h-8 px-4 z-20 bg-transparent text-sm text-white-stroke cursor-pointer select-none transition-colors",
              {
                "text-white-text":
